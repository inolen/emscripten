...
// libwasmfs.a.
  ...
  __syscall_poll: (fds, nfds, timeout) => {
    ...
    function dopoll(createWakepoll) {
      let count = 0;
      ...
      for (let i = 0; i < nfds; i++) {
        ...
        if (stream) {
          if (stream.stream_ops.poll) {
            const wakepoll = createWakepoll ? createWakepoll(events) : null;
            flags = stream.stream_ops.poll(stream, wakepoll);
          } else {
    ...
    if (isAsyncContext) {
      return new Promise((resolve) => {
        ...
        const count = dopoll(createWakepoll);
        let awakened = false;
        let timer = null;
